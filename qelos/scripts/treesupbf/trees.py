...
class NodeTrackerDF(object):
    ...
    """
    def __init__(self, root, with_struct=False):
        # !!! node name suffixes starting with * are ignored by Tree.parse()
        super(NodeTrackerDF, self).__init__()
        self.root = root
        self.possible_paths = []    # list of stacks
        self._nvt = None
        self.with_struct = with_struct
        self.start()
    ...
    def nxt(self, inpx, alt_x=None):       # x is a string or nothing
            ...
            while j < len(self.possible_paths):     # for every possible path
                ...
                while i < len(possible_path[-1]):   # look at bottom of stack
                    ...
                    if x == node.symbol(with_label=True, with_annotation=False, with_order=False) \
                        ...
                        new_possible_path[-1] = possible_path[-1] + [] # replace top with copy
                        del new_possible_path[-1][i]   # delete matched node from top

                        if len(new_possible_path[-1]) == 0:    # if top of stack empty, remove
