...
def get_all_reps(glovedim=300, merge_mode="cat", rel_which=("urlwords",), rel_embdim=None):
    gloveemb = q.PretrainedWordEmb(glovedim, incl_maskid=False, incl_rareid=False)
    question_sm, query_sm, qids, tx_sep = load_questions_inone()
...
def get_fl_emb(fl_sm, dim, gloveemb, computedwhat=q.ComputedWordEmb,
    ...
    if computedwhat == q.ComputedWordEmb:
        baseemb = q.WordEmb(dim=dim, worddic=fl_sm.D)       # base embedding (to be overridden by computed embeddings for entities and relations)
    elif computedwhat == q.ComputedWordLinout:
        baseemb = q.WordLinout(indim=dim, worddic=fl_sm.D)
    else:
        raise q.SumTingWongException()
    ent_emb = get_ent_emb(dim, gloveemb, mode=ent_merge_mode, computedwhat=computedwhat)
    rel_emb = get_rel_emb(dim, gloveemb, mode=rel_merge_mode, computedwhat=computedwhat,
...
def get_ent_emb(dim, gloveemb, mode="cat", computedwhat=q.ComputedWordEmb):
    ...
    if mode == "cat":
        dim = dim // 2
    entdic, entinfo = load_entity_info_mats()
